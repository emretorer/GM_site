...
import OgrenciPanelSidebar, { ogrenciNavItems } from '../components/OgrenciPanelSidebar.jsx'
import { signOutAndClearToken } from '../api/index.js'
import '../styles/panel.css'
import '../styles/panel-pages/ogrenci-panel.css'
import { initPanelInteractions } from '../scripts/panelInteractions.js'

function OgrenciPanelLayout() {
  ...
  const showSidebar = !location.pathname.startsWith('/ogrenci-panel/login')
  ...
  return (
    <div className="panel-root ogrenci-panel-root">
      {showSidebar ? (
        <OgrenciPanelSidebar isExpanded={isExpanded} onToggle={() => setIsExpanded(prev => !prev)} />
      ) : null}
      {showSidebar ? (
        <button
          type="button"
        ...
        <div className={`mobile-router-overlay${isDrawerClosing ? ' closing' : ''}`} onClick={closeDrawer}>
          <div className="mobile-router-sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <nav className="mobile-router-nav">
              {ogrenciNavItems.map((item, index) => (
                <NavLink
                  key={item.to}
                  to={item.to}
...
export default OgrenciPanelLayout
